...
function calculerAbattementsPV(annees: number) {
  ...
  if (annees <= 21)
    return { ir: (annees - 5) * 0.06, ps: (annees - 5) * 0.0165 };
  if (annees === 22) return { ir: 1.0, ps: 0.28 };
  if (annees <= 30)
    return { ir: 1.0, ps: Math.min(0.28 + (annees - 22) * 0.09, 1.0) };
  return { ir: 1.0, ps: 1.0 };
}

function calculerDureeRenteViagere(
  capital: number,
  ...
  const dureeAns =
    -Math.log(1 - (tauxNet * capital) / renteAnnuelle) / Math.log(1 + tauxNet);
  return { ans: Math.floor(dureeAns), mois: Math.floor((dureeAns % 1) * 12) };
}
...
export function calculateSimulation(
  ...
  let avCapital = params.avActif
    ? capitalTotal * (params.avAlloc / 100) * (1 - params.avFrais / 100)
    : 0;
  let scpiCapital = params.scpiActif
    ? capitalTotal * (params.scpiAlloc / 100) * (1 - scpiData.fraisEntree / 100)
    : 0;
  let immoValeur = params.immoActif
    ? capitalTotal * (params.immoAlloc / 100)
    : 0;
  let sp500 = params.actionsActif
    ? capitalTotal * (params.actionsAlloc / 100) * (params.sp500 / 100)
    : 0;
  let bitcoin = params.actionsActif
    ? capitalTotal * (params.actionsAlloc / 100) * ((100 - params.sp500) / 100)
    : 0;
  ...
  for (let annee = 0; annee <= DUREE; annee++) {
    ...
    const interetsAVBruts = params.avActif
      ? avCapital * (params.avRendement / 100)
      : 0;
    const fraisGestionAV = params.avActif
      ? avCapital * (params.avFraisGestion / 100)
      : 0;
    ...
    if (params.avActif && renteStoppeeAnnee === null && avCapital > 0) {
      if (avCapital >= renteAnnuelle) {
        ...
        const quotiteCapital =
          totalAV > 0 ? renteAnnuelle * (avCapital / totalAV) : 0;
    ...
    const lombardEnCours =
      params.lombardActif &&
      annee >= params.lombardAnnee &&
      anneesDepuisLombard < params.lombardDuree;
    ...
    const dividendesSCPIBruts =
      params.scpiActif && annee > 0
        ? (scpiCapital * scpiData.tauxBrut) / 100
        : 0;
    const fraisGestionSCPI = params.scpiActif
      ? (scpiCapital * scpiData.fraisGestion) / 100
      : 0;
    const dividendesSCPINets = Math.max(
      0,
      dividendesSCPIBruts - fraisGestionSCPI
    );
    let fiscaliteSCPI = 0;
    ...
    const loyersAnnuelsBruts = params.immoActif
      ? immoValeur * (annee === 0 ? 0 : params.immoRdt / 100)
      : 0;
    const loyersImposables =
      loyersAnnuelsBruts * (1 - FISCALITE.lmnpAbattement);
    const fiscaliteImmo =
      loyersImposables * (FISCALITE.lmnpIR + FISCALITE.lmnpPS);
    const loyersNets = Math.max(
      0,
      loyersAnnuelsBruts - params.taxeFonciere - fiscaliteImmo
    );
    ...
    // Calculer les gains AVANT de les ajouter au capital pour cohérence avec detailsFlux
    const gainsSP500 = params.actionsActif
      ? sp500 * (params.sp500Rdt / 100)
      : 0;
    const gainsBitcoin = params.actionsActif
      ? bitcoin * (params.bitcoinRdt / 100)
      : 0;
    sp500 += gainsSP500;
    ...
    resultatsAnnuels.push({
      ...
      detteLombard,
      patrimoineGlobal,
      patrimoineNet: patrimoineGlobal - detteLombard,
      loyersNets,
      dividendesSCPI: dividendesSCPIApresImpot,
      interetsLombard,
      fiscaliteAnnee: fiscaliteRente + fiscaliteImmo + fiscaliteSCPI,
      renteAnnuelle: renteEffective,
      remboursementLombard,
    });
    detailsFlux.push({
      annee,
      loyersBruts: loyersAnnuelsBruts,
      impotsLMNP: fiscaliteImmo,
      taxeFonciere: params.taxeFonciere,
      loyersVersPEL: loyersNets,
      dividendesSCPI: dividendesSCPIBruts,
      fraisGestionSCPI,
      flatTaxSCPI: fiscaliteSCPI,
      interetsAV: interetsAVNets,
      fraisGestionAV,
      rentesAV: renteEffective,
      flatTaxAV: fiscaliteRente,
      gainsSP500,
      gainsBitcoin,
  ...
  if (params.venteImmo && params.immoActif) {
    ...
    fiscalitePVImmo =
      pvImposableIR * FISCALITE.pvImmoIR + pvImposablePS * FISCALITE.psSeul;
    immoFinal = dernierResultat.immoValeur - fiscalitePVImmo;
    fiscaliteCumulee += fiscalitePVImmo;
  }

  const plusValueSP500 = params.actionsActif
    ? Math.max(
        0,
        dernierResultat.sp500 -
          capitalTotal * actionsAllocPct * (params.sp500 / 100)
      )
    : 0;
  const fiscaliteSP500 = plusValueSP500 * FISCALITE.psSeul;
  const plusValueBitcoin = params.actionsActif
    ? Math.max(
        0,
        dernierResultat.bitcoin -
          capitalTotal * actionsAllocPct * ((100 - params.sp500) / 100)
      )
    : 0;
  ...
  const totalAllocPct =
    (params.avActif ? params.avAlloc : 0) +
    (params.scpiActif ? params.scpiAlloc : 0) +
    (params.immoActif ? params.immoAlloc : 0) +
    (params.actionsActif ? params.actionsAlloc : 0) +
    (params.perActif ? params.perAlloc : 0);
  const capitalNonAlloue = capitalInitial * (1 - totalAllocPct / 100);

  let patrimoineFinalNet =
    dernierResultat.avCapital +
    dernierResultat.scpiCapital +
    immoFinal +
    bourseNette +
    dernierResultat.pelSolde +
    dernierResultat.perSolde -
    dernierResultat.detteLombard +
    capitalNonAlloue;
  ...
  const capitalPourRente = params.avActif
    ? capitalTotal * (params.avAlloc / 100) * (1 - params.avFrais / 100)
    : 0;
  const dureeRenteTotale = params.avActif
    ? calculerDureeRenteViagere(
        capitalPourRente,
        params.rente * 12,
        params.avRendement / 100,
        params.avFraisGestion / 100
      )
    : { ans: 0, mois: 0 };
  const anneesConsommees =
    params.rente > 0 ? rentesCumulees / (params.rente * 12) : 0;

  let dureeRestante = { ans: 0, mois: 0 };
  if (dureeRenteTotale.ans < 999 && params.avActif) {
    const dureeRestanteAns = Math.max(
      0,
      dureeRenteTotale.ans + dureeRenteTotale.mois / 12 - anneesConsommees
    );
    dureeRestante = {
      ans: Math.floor(dureeRestanteAns),
      mois: Math.floor((dureeRestanteAns % 1) * 12),
    };
  } else if (params.avActif) {
  ...
  return {
    ...
    resultatsAnnuels: resultatsAnnuels.map(
      (r) =>
        Object.fromEntries(
          Object.entries(r).map(([k, v]) => [k, Math.round(v)])
        ) as unknown as AnnualResult
    ),
    // Fix: Use 'as unknown as Type' to fix TypeScript casting error when dynamically creating an object after rounding values.
    detailsFlux: detailsFlux.map(
      (f) =>
        Object.fromEntries(
          Object.entries(f).map(([k, v]) => [k, Math.round(v)])
        ) as unknown as FluxDetail
    ),
    synthese: {
      capitalInitial,
      patrimoineFinalNet,
      patrimoineFinalNetReel,
      plusValueTotale,
      plusValueTotaleReelle,
      plusValuePct:
        capitalInitial > 0
          ? ((plusValueTotale / capitalInitial) * 100).toFixed(2)
          : "0.00",
      plusValuePctReelle:
        capitalInitial > 0
          ? ((plusValueTotaleReelle / capitalInitial) * 100).toFixed(2)
          : "0.00",
      rentesCumulees,
      rentesCumuleesNettes,
      dividendesSCPICumules,
      pelFinal: dernierResultat.pelSolde,
      perFinal: dernierResultat.perSolde,
      economiesIRPER,
      interetsLombardTotaux: interetsLombardCumules,
      fiscaliteTotale: fiscaliteCumulee,
      rendementAnnuelMoyen:
        Math.abs(plusValueTotale) > 1 && capitalInitial > 0
          ? ((plusValueTotale / capitalInitial / DUREE) * 100).toFixed(2)
          : "0.00",
      rendementAnnuelMoyenReel: params.inflationActif
        ? Math.abs(plusValueTotaleReelle) > 1 && capitalInitial > 0
          ? ((plusValueTotaleReelle / capitalInitial / DUREE) * 100).toFixed(2)
          : (-params.inflationTaux).toFixed(2)
        : Math.abs(plusValueTotale) > 1 && capitalInitial > 0
        ? ((plusValueTotale / capitalInitial / DUREE) * 100).toFixed(2)
        : "0.00",
      detteFinal: dernierResultat.detteLombard,
      fiscalitePVImmo,
      fiscaliteSP500,
      fiscaliteBitcoin,
      ...
      anneesConsommees,
      montantLombardEmprunte,
      coutLombardTotal: interetsLombardCumules,
      capitalAVFinal: dernierResultat.avCapital,
      capitalAVDepart: capitalPourRente,
      renteStoppee: renteStoppeeAnnee,
      renteInfinie: dureeRenteTotale.ans >= 999,
      inflationCumulee: params.inflationActif
        ? ((Math.pow(1 + params.inflationTaux / 100, DUREE) - 1) * 100).toFixed(
            2
          )
        : "0",
      capitalNonAlloue,
