...
const DarkModeToggle: React.FC<{ darkMode: boolean; toggle: () => void; }> = ({ darkMode, toggle }) => (
    <button
        onClick={toggle}
...
const Loader: React.FC<{ isLoading: boolean }> = ({ isLoading }) => (
    <div className={`absolute top-4 sm:top-6 right-4 sm:right-6 w-10 h-10 transition-opacity duration-300 ${isLoading ? 'opacity-100' : 'opacity-0'}`}>
        <div className="w-8 h-8 border-4 border-white/30 border-t-white rounded-full animate-spin"></div>
    </div>
);

export const Header: React.FC<HeaderProps> = ({ darkMode, toggleDarkMode, isLoading }) => (
    <header className="bg-gradient-to-br from-primary to-secondary text-white p-8 text-center relative">
        ...
        <h1 className="text-3xl md:text-4xl font-extrabold mb-2">🏦 Simulateur Patrimonial Expert</h1>
        <p className="text-base md:text-lg opacity-90">Ingénierie patrimoniale & fiscalité française 2025 • Vos allocations dans 3 scénarios</p>
    </header>
);
